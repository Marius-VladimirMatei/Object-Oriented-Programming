...
class AccountView:
    ...
    def create_widgets(self) -> None:
        ...
        self.create_account_form = CreateAccountView(self.create_tab, self.controller, self.refresh_accounts)
    ...
    def create_accounts_list(self) -> None:
        ...
        ttk.Entry(search_frame, textvariable=self.search_var).grid(row=0, column=1, padx=5, sticky="ew")
        ...
        # Add scrollbars
        y_scrollbar = ttk.Scrollbar(self.accounts_tab, orient="vertical", command=self.tree.yview)
        x_scrollbar = ttk.Scrollbar(self.accounts_tab, orient="horizontal", command=self.tree.xview)
        ...
        self.amount_var = tk.StringVar()
        ttk.Entry(trans_frame, textvariable=self.amount_var).grid(row=0, column=1, padx=5, pady=5, sticky="ew")

        # Buttons frame
        btn_frame = ttk.Frame(trans_frame)
        btn_frame.grid(row=0, column=2, padx=5, pady=5)
        ttk.Button(btn_frame, text="Deposit", command=self.deposit).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="Withdraw", command=self.withdraw).pack(side=tk.LEFT, padx=2)

        # Account management buttons
        mgmt_frame = ttk.Frame(trans_frame)
        mgmt_frame.grid(row=0, column=3, padx=5, pady=5)
        ttk.Button(mgmt_frame, text="Edit", command=self.edit_account).pack(side=tk.LEFT, padx=2)
        ttk.Button(mgmt_frame, text="Delete", command=self.delete_account).pack(side=tk.LEFT, padx=2)
    ...
    def perform_transaction(self, transaction_type: str) -> None:
        ...
        try:
            ...
            self.controller.perform_transaction(account_number, amount, transaction_type)
